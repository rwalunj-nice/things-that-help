...
description: Use when running GitHub Copilot usage analysis. Collects inputs (R&D VP, Product, Usage date, Team) one at a time, then guides Power BI data extraction and runs all 4 framework analyses (v1, v2, Role+Momentum, CRQC), writing output files and pushing to the copilot-projects repo.
---
...
| User | Int | LoC Added | LoC Sugg | Code Accept | Code Gen | % Accept | Sugg Eff | Prem Req | Req Eff | Agent Contribution % |

Ask the user to paste or screenshot the table. Capture all rows, including users with zero activity.

### Step 2: PR Details data
